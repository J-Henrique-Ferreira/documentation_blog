<tr class="hover:bg-slate-50 w-full">
    <td class="p-4 max-w-[800px]">
        ...
        <div
            class="text-sm text-slate-500 mt-1 text-ellipsis overflow-hidden [display:-webkit-box] [-webkit-line-clamp:2] [-webkit-box-orient:vertical]">
            {{ $post->description }}
        </div>
    </td>
    ...
    <td class="p-4 hidden sm:table-cell">
        ...
        @if($post->status === 'draft')
            <span
                class="inline-flex items-center gap-1 px-2 py-1 bg-yellow-200 text-yellow-700 rounded text-xs font-medium">
                <i class="fa fa-check-circle"></i>
                rascunho
            </span>
        @endif
    ...
    <td class="p-4">
        <div class="flex items-center justify-end gap-1">
            ...
            <form onsubmit="return confirm('Confirmar remover documento?')"
                action="{{ route('admin.documentos.destroy', $post->id) }}" method="POST">
                @csrf
                @method('DELETE')
                <button type="submit"
